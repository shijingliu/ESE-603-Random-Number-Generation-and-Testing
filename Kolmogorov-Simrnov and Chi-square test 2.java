...
public class Problem5 {
      ...
      //generate numbers according to formulas                
      public static double [] generateNumbers(){
    	  double [] genNum = new double [100];   
    	  //set up the initial number R0   
    	  genNum[0] = 0.1234; 
    	  
    	  //generate the following sequence    
    	  for (int i = 0; i < 99; i++){
    		  if((genNum[i]<0.5) && (genNum[i] >=0.0)){       
    			    double temp = 2.0*genNum[i];
    			    genNum[i+1] = Math.round(temp*10000.0)/10000.0;
    		  }else if ((genNum[i] >= 0.5) && (genNum[i] < 1)){
    			    double temp = 2.0*(1.0-genNum[i]);
    			    genNum[i+1] = Math.round(temp*10000.0)/10000.0; 
    		  }else{        
    		  }
    	  }
